...
* Inbound XML deserialization (`ToC2SIMObject<T>`) now tolerates empty elements. Real producers (e.g. VR-Forces) emit empty elements such as `<OperationalStatusCode></OperationalStatusCode>` to mean "unspecified"; the strict `XmlSerializer` rejected these (`'' is not a valid value for OperationalStatusCodeType`), so a majority of real reports could not be parsed. Empty elements (no attributes, no children, whitespace-only text) - including containers left empty by the strip - are now treated as absent and removed before deserialization; a fully-empty subtree collapses so the parent property deserializes to null (an honest "unspecified") instead of a phantom default value. Populated values are untouched
